...
class PlaywrightController:
    ...
    async def gradual_cursor_animation(
        ...
        Args:
            ...
            steps (int, optional): Number of steps for the animation. Default: 20
            step_delay (float, optional): Delay between steps in seconds. Default: 0.05
        """
        await self._ensure_page_ready(page)
        await self._animation.gradual_cursor_animation(
            page, start_x, start_y, end_x, end_y, steps, step_delay
        )
    ...
    async def cleanup_animations(self, page: Page) -> None:
        """
        Clean up all animations and visual effects on the page.
    ...
    async def show_marked_elements(self, page: Page) -> Dict[str, Any]:
        """
        Show all elements on the page that have __elementId attribute with tooltips.
        This is useful for exploring what elements are marked and available for interaction.
        
        Args:
            page (Page): The Playwright page object.
            
        Returns:
            Dict[str, Any]: Information about the marked elements.
        """
        await self._ensure_page_ready(page)
        return await self._animation.show_all_marked_elements(page)
        
    async def hide_marked_elements(self, page: Page) -> None:
        """
        Hide all marked elements that were shown with show_marked_elements.
        
        Args:
            page (Page): The Playwright page object.
        """
        await self._ensure_page_ready(page)
        await self._animation.hide_all_marked_elements(page)
        
    async def add_action_effect(
        self, 
        page: Page, 
        action_type: str, 
        ...
        """
        Add a visual effect for a particular action without actually performing the action.
        Used to emphasize what action is being performed during demonstrations.
        
        Args:
            page (Page): The Playwright page object.
            action_type (str): Type of action ('click', 'hover', 'type', etc.)
            element_id (Optional[str]): Element ID if action is on an element.
            coords (Optional[Tuple[float, float]]): X,Y coordinates if action is at a position.
        """
        await self._ensure_page_ready(page)
        await self._animation.add_action_effect(page, action_type, element_id, coords)
        
    async def remove_action_effect(self, page: Page, action_type: str) -> None:
        """
        Remove a visual effect that was added with add_action_effect.
        
        Args:
            page (Page): The Playwright page object.
            action_type (str): Type of action to remove effect for ('click', 'hover', 'type', etc.)
        """
        await self._ensure_page_ready(page)
        await self._animation.remove_action_effect(page, action_type)
